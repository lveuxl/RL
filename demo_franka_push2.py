...
def animate_franka_pushing_with_planner(env, planner, selected_actor, extraction_distance=0.15):
    """
    ...
    try:
        # === 阶段1: 移动到接触点前方的预备位置 ===
        # 宏观移动, 直接调用 Planner 一次性完成, follow_path 内部自动 env.step()
        ...
        if result == -1:
            print(f"   ⚠️ 无法到达接触点，但继续尝试推动")

        _hold_steps(env, 10)

        # === 阶段3: 一次规划 + 分段执行同步推动 ===
        # 只调用一次 plan_screw 规划从接触点到推动终点的完整轨迹,
        # 然后按轨迹步数等分木块位移, 逐步执行并同步更新木块 Pose。
        final_push_pos = contact + push_dir * extraction_distance
        final_push_pose = sapien.Pose(p=final_push_pos, q=push_pose.q)

        trajectory = planner.move_to_pose_with_screw(final_push_pose, dry_run=True)
        if trajectory == -1:
            print(f"   ⚠️ 无法规划推动轨迹, 跳过此木块")
            return

        n_traj = trajectory["position"].shape[0]
        print(f"   → Phase 3: 同步推动木块 ({extraction_distance}m, 轨迹 {n_traj} 步)")

        initial_p = selected_actor.pose.p.clone()
        total_block_delta = torch.tensor(
            push_dir * extraction_distance, device=device
        ).unsqueeze(0)

        for i in range(n_traj):
            # 3a: 按轨迹进度同步更新木块 kinematic Pose
            progress = (i + 1) / n_traj
            target_p = initial_p + total_block_delta * progress
            selected_actor.set_pose(Pose.create_from_pq(target_p))

            # 3b: 执行轨迹中的单步 (复用已规划好的关节位置)
            qpos = trajectory["position"][i]
            action = np.hstack([qpos, planner.gripper_state])
            env.step(action)

        # === 阶段4: 木块消失 ===
        print(f"   → Phase 4: 移除木块")
