            ...
            if stage == 0:
                ...
                if tick == 0:
                    ...
                    try:
                        ...
                        print(f"环境{env_idx}: Stage 2 - 创建吸盘约束异常: {e}")
                        pass
                # 等待物体稳定
                # if tick >= 5:  # 等待5步让物体稳定
                #     self.env_stage[env_idx] = 3
                ...
                if tick == 0:
                    # 定义固定的放置区域位置，避免跟随对象位置变化
                    drop_zone_pos = np.array([-0.4, 0.4, 0.05])  # 放置位置上方20cm (0.05+0.2=0.25)
                    self.stage_positions[env_idx] = torch.tensor(drop_zone_pos, device=self.device)

                target_pos = self.stage_positions[env_idx]
                action[:3], reached = self._get_move_action(tcp_pos, target_pos, max_steps=200)
                # if tick == 0:
                #     # 使用stage 3的xy坐标，但z坐标设为放置高度
                #     current_xy = self.stage_positions[env_idx][:2].cpu().numpy()
                #     target_pos = np.array([current_xy[0], current_xy[1], 0.05])  # 下降到桌面高度5cm
                #     self.stage_positions[env_idx] = torch.tensor(target_pos, device=self.device)
                
                # target_pos = self.stage_positions[env_idx]
                # action[:3], reached = self._get_move_action(tcp_pos, target_pos, max_steps=100)

                if reached or tick >= 200:
