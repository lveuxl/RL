    ...
    **Success Conditions:**
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            # 加噪声：XY 平移扰动 ±2mm，Yaw 旋转扰动 ±2°
            # xy_noise = (torch.rand(self.num_levels, 3, b, 2, device=self.device) - 0.5) * 0.004
            # yaw_noise = (torch.rand(self.num_levels, 3, b, device=self.device) - 0.5) * (4 * torch.pi / 180)
            # half_yaw = yaw_noise * 0.5
            # q_noise = torch.stack([torch.cos(half_yaw), torch.zeros_like(half_yaw),
            #                        torch.zeros_like(half_yaw), torch.sin(half_yaw)], dim=-1)
            ...
            for level in range(self.num_levels):
                ...
                for i in range(3):
                    ...
                    # 加噪声：
                    # xyz = torch.tensor([pos], device=self.device).expand(b, -1).clone()
                    # xyz[:, :2] += xy_noise[level, i]
                    # self.blocks[idx].set_pose(Pose.create_from_pq(xyz, q_noise[level, i]))
